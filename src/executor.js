...
class Executor {
	run(options) {
		...
		switch (options.mode) {
			...
			case "deploy-cloud": {
				this.deployCloud(options).then(fnInfo => this.prepareLocal());
				break;
			}
			case "prepare-local": {
		...
		return new Promise((resolve, reject) => {
			fs.stat(dir, (err, stats) => {
				if (err) {
					...
					reject(
						`The directory ${dir} already exists, please provide a new directory`
					);
				}
			});
		});
			...
			setTimeout(
				() => {
					...
					const getUri = stdout => {
						...
						for (let i = 0; i < info.length; i++) {
							if (info[i] == "endpoints:") {
								...
								const uri = endpoints.slice(9, endpoints.length);
								return uri;
							}
						}
					};
					...
					Promise.all(promises).then(() => {
						console.log(
							`Done deploying, collected: ${JSON.stringify(name_uri)}`
						);
						resolve(name_uri);
					});
				},
